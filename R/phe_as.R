phe_as <-
  function(phe.gen, additive.genotypes=T,min.records=20,return.models=F,confint.level=NA, my.data, my.covariates, chunk.size) {
    require(biglm)
    ...
    if(n_total<min.records) {
      ...
      if(class(d[[phe]]) %in% c("logical")) {
        ...
        if(n_cases<min.records|n_controls<min.records) {note=paste(note,"[Error: <",min.records," cases or controls]")}
          ...
          model = bigglm(as.formula(paste(phe,"~", gen,"+", paste(cov, collapse = '+'), sep = '')), data=d, family=binomial(), weights=~weight, chunksize=chunk.size)
          modsum= summary(model)
          #Find the rows with results that gets merged across all loops
          gen_list=grep(gen,row.names(modsum$mat))
          gens=row.names(modsum$mat)[gen_list]
          or=exp(as.numeric(modsum$mat[gen_list,1]))
          beta=as.numeric(modsum$mat[gen_list,1])
          se=as.numeric(modsum$mat[gen_list,4])
          p=as.numeric(modsum$mat[gen_list,5])
        }
      } else {
        type = "linear"
        if(n_total<min.records) {
          ...
          model = biglm(as.formula(paste(phe,"~", gen,"+", paste(cov, collapse = '+'), sep = '')), data=d, weights=~weight, chunksize=chunk.size)
          modsum= summary(model)
          #Find the rows with results that gets merged across all loops
          gen_list=grep(gen,row.names(modsum$coef))
          gens=row.names(modsum$mat)[gen_list]
          beta=modsum$mat[gen_list,1]
          se=modsum$mat[gen_list,4]
          p=modsum$mat[gen_list,5]
        }
      }
    }
